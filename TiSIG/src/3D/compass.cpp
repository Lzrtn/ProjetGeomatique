...
CompassFactory::CompassFactory()
{
	...
	int n=48; // resolution circle

	for (int i=0; i<n; i++) {
		for (int k=0; k<n; k++) {
			QVector3D p1 = makePtForCompas(d, 2.0*M_PI*  i  /n, e, 2.*  k  /n*M_PI);
			QVector3D p2 = makePtForCompas(d, 2.0*M_PI*  i  /n, e, 2.*(k+1)/n*M_PI);
			QVector3D p5 = makePtForCompas(d, 2.0*M_PI*(i+1)/n, e, 2.*  k  /n*M_PI);
			QVector3D p6 = makePtForCompas(d, 2.0*M_PI*(i+1)/n, e, 2.*(k+1)/n*M_PI);

			this->position.push_back(p1);
			this->position.push_back(p2);
			this->position.push_back(p6);
			this->position.push_back(p1);
			this->position.push_back(p5);
			this->position.push_back(p6);
